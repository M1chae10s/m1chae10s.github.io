...
let coin = new Subject(0, 0, "./pic/russianMario/coin.png");
coin.setY(200);
...
function defaultPosition() {
    coin.randomX();
    player.setX(50);
    player.setY(innerHeight - 200);
    guard.setX(window.innerWidth - 250);
    guard.setY(innerHeight - 350);
...
function draw() {
    if (player.getDirection()) {
        imgPlayer.src = "./pic/russianMario/playerRight.png";
    } else {
        imgPlayer.src = "./pic/russianMario/playerLeft.png";
    }
...
function gameOver() {
    if (guard.getPhase()) {
        if (player.getX() - 50 <= guard.getX() - 100) {
            if (player.getX() + 50 >= guard.getX() - 100) {
                if (player.getY() - 150 <= guard.getY()) {
                    if (player.getY() + 50 >= guard.getY()) {
...
function jumpDown() {
    {
        setTimeout(function () {
            if (player.getY() < window.innerHeight * 0.75) {
               player.setY(player.getY()+5);
            } else {
                isJump = true;
                draw();
